...
export const getAllCourts = async (c: Context) => {
    try {
        ...
        logger.error({ error: err }, "Failed to fetch courts");
        return error(c, "Failed to fetch courts", 500);
    }
};
...
export const getCourtById = async (c: Context) => {
    try {
        ...
        logger.error({ error: err }, "Failed to fetch court");
        return error(c, "Failed to fetch court", 500);
    }
};
    ...
    try {
        ...
        if (data.image && data.image instanceof File) {
            try {
                ...
                logger.error({ error: uploadErr }, "Failed to upload court image");
                return error(c, "Failed to upload image. Please try again later.", 500);
            }
        }
        ...
        logger.error({ error: err }, "Failed to create court");
        return error(c, "Failed to create court", 500);
    }
};
    ...
    try {
        ...
        if (data.image && data.image instanceof File) {
            if (existingCourt.imageKey) {
                await deleteImage(existingCourt.imageKey).catch((err: unknown) => logger.error({ error: err }, "Failed to delete old court image"));
            }
        ...
        logger.error({ error: err }, "Failed to update court");
        return error(c, "Failed to update court", 500);
    }
};
...
export const deleteCourt = async (c: Context) => {
    try {
        ...
        logger.error({ error: err }, "Failed to delete court");
        return error(c, "Failed to delete court", 500);
    }
};
